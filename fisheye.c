	...
	for (currentY = 0; currentY < height; currentY++) {
		...
		for (currentX = 0; currentX < width; currentX++) {
			...
			if (distanceToCenter >= 0.0 && distanceToCenter <= 1.0) {
				double newDistanceToCenter = (distanceToCenter + (1.0 - sqrt(1.0 - distanceToCenter*distanceToCenter))) / 2.0;
				...
				if (newDistanceToCenter <= 1.0) {
                    ...
                    if (sourcePosition >= 0 & sourcePosition < width * height) {
                        ...
                        newData[destinationPosition].red = data[sourcePosition].red;    
                        newData[destinationPosition].green = data[sourcePosition].green;    
                        newData[destinationPosition].blue = data[sourcePosition].blue;    
                    }
				}
			}
			else {
        	  	newData[currentY * width + currentX].red = 0;
    	      	newData[currentY * width + currentX].green = 0;
	          	newData[currentY * width + currentX].blue = 0;
            }
		}
	}
